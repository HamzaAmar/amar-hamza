...
If you look at this example carefully, you will see that we have conflict in the background declarations,and one of the declaration within an ID selector, which means you can’t override it no matter how many classes or other selectors you add. That means the focus and disabled background never applied. You can override the ID selector only by either ID or `!important`. This is why people suggest that using ID as a selector is a bad habit to fix this issue. Let’s replace ID with `btn-gmail` class selector.

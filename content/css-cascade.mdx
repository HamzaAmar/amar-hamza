...
<Alert
  color="p"
  message="The !important behavior is not exclusive to the Origins layer but also applies to other layers such as Context and Cascade Layer, which we will discuss in the next CSS cascade sections."
...
- Remember the rule we said before about `!important`, when it is added to a declaration it's introduces new stages above the normal ones in a reversed order.
...
- each number has a selector(s) that increases its value.

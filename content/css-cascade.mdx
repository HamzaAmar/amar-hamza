...
by identifying styles that don't have any conflicts; they will apply directly
to the selected element(s) without the need for cascade resolution.
...
- By merging layers with the same name, we can use this feature to reorder the under layers to be clear and simple to Visualize the priority of each layer without scrolling the entire stylesheet, look at the example below, where we use `@layer reset components utils;` to reorder layers.
...
**Use cascade layers (`@layer`)**: wrap the reset styles in a reset layer, this solution was designed for this purpose. but we want other solutions, because similar issues can occur whithin a layer, and creating new layer for every issues isn't practical, we need other strategies.
...
You could reduce the specificity by removing one or more of the selector, if possible and doesn't affect the selected element. Like in this example if we remove `[type='submit']`, `[type='button']` it will affect all input elements. but our reset only aim for input buttons either `button` or `submit` so we can't remove the selector(s).

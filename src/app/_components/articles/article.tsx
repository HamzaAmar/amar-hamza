import { Avatar, Button, Flex, Tag, Text } from "@components/core";
...
export const Article = ({ excerpt, title, slug, tags, image, publishedAt }: ArticleProps) => {
  const id = useId();
  const buttonId = `${id}-b`;
      ...
      <div className="ar-C Sf-2">
        <Flex items="center">
          <Flex gap="4" items="center">
            <Avatar image="/me.jpg" title="my Avatar" loading="lazy" />
            <div>
              <Text size="3" weight="5">
                Hamza Miloud Amar
              </Text>
              <Text low color="b" as="time" size="3" dateTime={publishedAt}>
                {formatDate(publishedAt)}
              </Text>
            </div>
          </Flex>
        </Flex>
        <Text type="heading" id={headingId} as="h3" align="start" weight="5" truncate="2">
          {title}
        </Text>

...
import { Button, Flex, IconButton, Social, Spinner, Text } from "@components/core";
...
const Item = ({ name, path, mobile, handleClose }: ItemProps) => {
  ...
  return (
    <li className="h-e-itm" data-mobile={mobile}>
      <NavLink className="Fc h-e-lnk" data-mobile={mobile} {...close} {...onlyDesktop} href={path}>
        {name}
      </NavLink>
    </li>
  );
...
const Header = () => {
  ...
  return (
    <Flex justify="between" items="center" as="header" className="h-e">
      <Flex as={Link} aria-label="Home Page" items="center" gap="4" href="/" className="h-e-logo">
        <Logo width="140" />
      </Flex>
      <nav className="h-e-nav sm_hide u_flex-1">
        <ul className="Fc h-e-lst">
          {MENU.map(({ id, name, path }) => {
            return <Item key={id} name={name} path={path} pathname={path} />;
          })}
        </ul>
      </nav>
      ...
      <nav className="h-e-mob Sf-8" data-visible={visible === true}>
        ...
        <ul className="h-e-lstM u_flex-1">
          {MENU.map(({ id, name, path }) => {
            return <Item key={id} name={name} path={path} pathname={pathname} mobile handleClose={handleFalse} />;
          })}
        </ul>
        ...
        <div className="Sf-6">
          ...
          <Text align="center" weight="1" size="2" color="b" low>
            I'm glad you're here! Explore my work and get to know me better.
          </Text>
        </div>
      </nav>
    </Flex>
